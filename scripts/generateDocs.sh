...
if [ -d "docs/@moveindustries/ts-sdk-$npm_package_version" ]; then
...
npx typedoc src/index.ts --options typedoc.json --out "docs/@moveindustries/ts-sdk-$npm_package_version" --plugin typedoc-plugin-missing-exports --internalModule PrivateCode --cleanOutputDir --excludeInternal --includeVersion --skipErrorChecking
...
if [ -n "$LINE" ]; then
  ...
  # TODO: Make this more stable / a different way of generating that isn't just inserting a line
  {
    ...
    echo "- [$LABEL - @moveindustries/ts-sdk-$npm_package_version](@moveindustries/ts-sdk-$npm_package_version)";
    tail -n +$NEXT_LINE $INDEX_FILE;
  } > $INDEX_FILE.tmp && mv $INDEX_FILE.tmp $INDEX_FILE
fi

# Now update the redirect
REDIRECT_FILE='docs/@moveindustries/ts-sdk-latest/index.md';
