...
async function main() {
  ...
  for (let i = 0; i < senders.length; i += 1) {
    funds.push(
      movement.fundAccount({ accountAddress: senders[i].accountAddress.toStringWithoutPrefix(), amount: 10000000000 }),
    );
  }
  ...
  for (let i = 0; i < senders.length; i += 1) {
    balances.push(movement.getAccountInfo({ accountAddress: senders[i].accountAddress }));
  }
  ...
  console.log(`sends ${totalTransactions * senders.length} transactions to ${movement.config.network}....`);
  // emit batch transactions
  senders.map((sender) => movement.transaction.batch.forSingleAccount({ sender, data: payloads }));
  ...
  movement.transaction.batch.on(TransactionWorkerEventsEnum.ExecutionFinish, async (data) => {
    ...
    const accounts = senders.map((sender) => movement.getAccountInfo({ accountAddress: sender.accountAddress }));
