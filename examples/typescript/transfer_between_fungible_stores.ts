...
async function createSecondaryStore(admin: Account, user: Account, metadata: AccountAddress): Promise<string> {
  ...
  const senderAuthenticator = movement.transaction.sign({ signer: user, transaction });
  const pendingTxn = await movement.transaction.submit.simple({ transaction, senderAuthenticator });
...
async function main() {
  try {
    ...
    // Fund accounts
    await Promise.all([
      movement.fundAccount({ accountAddress: alice.accountAddress, amount: 100_000_000 }),
      movement.fundAccount({ accountAddress: bob.accountAddress, amount: 100_000_000 }),
      movement.fundAccount({ accountAddress: charlie.accountAddress, amount: 100_000_000 }),
    ]);
    ...
    const transferToSecondaryTxn = await movement.transferFungibleAssetBetweenStores({
      sender: bob,
      fromStore: bobPrimaryStoreAddress,
      toStore: bobSecondaryStore,
      amount: 800_000_000,
    });
    const transferToSecondaryResponse = await movement.signAndSubmitTransaction({
      signer: bob,
      transaction: transferToSecondaryTxn,
    });
    await movement.waitForTransaction({ transactionHash: transferToSecondaryResponse.hash });
    ...
    const transferBetweenSecondaryTxn = await movement.transferFungibleAssetBetweenStores({
      sender: bob,
      fromStore: bobSecondaryStore,
      toStore: charlieSecondaryStore,
      amount: 600_000_000,
    });
    const transferBetweenSecondaryResponse = await movement.signAndSubmitTransaction({
      signer: bob,
      transaction: transferBetweenSecondaryTxn,
    });
    await movement.waitForTransaction({ transactionHash: transferBetweenSecondaryResponse.hash });
    ...
    const transferFromSecondaryTxn = await movement.transferFungibleAssetBetweenStores({
      sender: charlie,
      fromStore: charlieSecondaryStore,
      toStore: bobPrimaryStoreAddress,
      amount: 350_000_000,
    });
    const transferFromSecondaryResponse = await movement.signAndSubmitTransaction({
      signer: charlie,
      transaction: transferFromSecondaryTxn,
    });
    await movement.waitForTransaction({ transactionHash: transferFromSecondaryResponse.hash });
