...
import { EphemeralKeyPair, Movement, MovementConfig, Network } from "@moveindustries/ts-sdk";
...
const example = async () => {
  ...
  const jwkTxn = await movement.updateFederatedKeylessJwkSetTransaction({ sender: alice, iss });
  await movement.signAndSubmitTransaction({ signer: alice, transaction: jwkTxn });
