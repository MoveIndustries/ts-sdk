...
describe("transaction builder", () => {
  ...
  describe("generateSignedTransactionForSimulation", () => {
    ...
    test("it generates a keyless signed raw transaction for simulation", async () => {
      ...
      const pepper = await movement.getPepper({ jwt, ephemeralKeyPair });
      const publicKey = KeylessPublicKey.fromJwtAndPepper({ jwt, pepper });
      const accountAddress = publicKey.authKey().derivedAddress();
      await movement.fundAccount({ accountAddress, amount: FUND_AMOUNT });
      const payload = await generateTransactionPayload({
        bytecode: multiSignerScriptBytecode,
        functionArguments: [
          new U64(100),
  ...
  describe("generateUserTransactionHash", () => {
    test("it generates a single signer signed transaction hash", async () => {
      const alice = Account.generate();
      await movement.fundAccount({ accountAddress: alice.accountAddress, amount: FUND_AMOUNT });
      const transaction = await movement.coin.transferCoinTransaction({
        sender: alice.accountAddress,
        recipient: "0x1",
        amount: 1,
      });
      ...
      // Submit transaction
      const submitted = await movement.transaction.submit.simple(signedTxnInput);
    ...
    test("it generates a multi agent signed transaction", async () => {
      ...
      await movement.fundAccount({ accountAddress: alice.accountAddress, amount: FUND_AMOUNT });
      const bob = await Account.generate();
      await movement.fundAccount({ accountAddress: bob.accountAddress, amount: 1 });
      const payload = await generateTransactionPayload({
        movementConfig: config,
        function: `${contractPublisherAccount.accountAddress}::transfer::transfer`,
        functionArguments: [1, bob.accountAddress],
      ...
      // Submit transaction
      const submitted = await movement.transaction.submit.multiAgent(signedTxnInput);
      expect(submitted.hash).toBe(transactionHash);
    });
