...
describe("Registration Proof (ZKPoK of Decryption Key)", () => {
  ...
  it("generates a valid registration proof", () => {
    const dk = makeKey();
    const proof = genRegistrationProof(dk, chainId, senderAddress, tokenAddress);
  ...
  it("valid proof verifies successfully", () => {
    ...
    const valid = verifyRegistrationProof(ek, proof, chainId, senderAddress, tokenAddress);
  ...
  it("proof fails with wrong chain ID", () => {
    ...
    const valid = verifyRegistrationProof(ek, proof, 99, senderAddress, tokenAddress);
  ...
  it("proof fails with wrong sender address", () => {
    ...
    const valid = verifyRegistrationProof(ek, proof, chainId, wrongSender, tokenAddress);
    expect(valid).toBe(false);
  });
  ...
  it("proof fails with wrong token address", () => {
    ...
    const valid = verifyRegistrationProof(ek, proof, chainId, senderAddress, wrongToken);
    expect(valid).toBe(false);
  });
  ...
  it("proof fails with wrong encryption key", () => {
    ...
    const valid = verifyRegistrationProof(otherEk, proof, chainId, senderAddress, tokenAddress);
    expect(valid).toBe(false);
  });
  ...
  it("different keys produce different proofs", () => {
    const dk1 = makeKey();
    const dk2 = makeKey();
    const proof1 = genRegistrationProof(dk1, chainId, senderAddress, tokenAddress);
    const proof2 = genRegistrationProof(dk2, chainId, senderAddress, tokenAddress);
  ...
  it("same key produces different proofs each time (random nonce)", () => {
    const dk = makeKey();
    const proof1 = genRegistrationProof(dk, chainId, senderAddress, tokenAddress);
    const proof2 = genRegistrationProof(dk, chainId, senderAddress, tokenAddress);
    ...
    const ek = dk.publicKey().toUint8Array();
    expect(verifyRegistrationProof(ek, proof1, chainId, senderAddress, tokenAddress)).toBe(true);
    expect(verifyRegistrationProof(ek, proof2, chainId, senderAddress, tokenAddress)).toBe(true);
  });
});
